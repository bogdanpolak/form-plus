...
procedure TActionGuiBuilder.ClearActions;
begin
  FActions.Clear;
end;
...
begin
  Next := FActions.Count;
  FActions.Count := Next + Length(aActions);
  for i := 0 to Length(aActions) - 1 do
    FActions[Next+i] := aActions[i];
end;
...
begin
  Next := FActions.Count;
  FActions.Count := Next +  aActionList.ActionCount;
  for i := 0 to  aActionList.ActionCount - 1 do
    FActions[Next+i] := aActionList.Actions[i];
end;
